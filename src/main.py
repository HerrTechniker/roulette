...
class RouletteApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.app_config = load_json(CONFIG_FILE)
        if "ui_theme" not in self.app_config:
            self.app_config["ui_theme"] = "system"
            save_json(CONFIG_FILE, self.app_config)
        self.current_user: Optional[str] = None
        self.current_bets: List[Bet] = []
        self.result_number: Optional[str] = None
        self.theme_var = tk.StringVar(value=self.app_config.get("ui_theme", "system"))
    ...
    def on_theme_change(self) -> None:
        ...
        self.app_config["ui_theme"] = new_theme
        self.save_config()
...
class LoginFrame(tk.Frame):
    ...
    def admin_login(self) -> None:
        ...
        if not user or user.get("password_hash") != hash_password(password):
            messagebox.showerror("Login fehlgeschlagen", "Benutzername oder Passwort falsch.")
            return
        if username not in self.app.app_config.get("admin_users", []):
...
class RegisterFrame(tk.Frame):
    ...
    def register(self) -> None:
        ...
        if username in self.app.users:
            ...
            return
        start_balance = self.app.app_config.get("default_start_balance", 100.0)
...
class AdminFrame(tk.Frame):
    ...
    def refresh(self) -> None:
        self.balance_var.set(f"{self.app.app_config.get('default_start_balance', 100.0):.2f}")
    ...
    def save_balance(self) -> None:
        ...
        if amount <= 0:
            ...
            return
        self.app.app_config["default_start_balance"] = round(amount, 2)
